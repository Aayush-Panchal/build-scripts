...
if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
    ...
    curl -s -H "Authorization: bearer $token" \
      "https://s3.us.cloud-object-storage.appdomain.cloud/ose-power-artifacts-stag?list-type=2&prefix=$PACKAGE_NAME/$VERSION/%22 \
    ...
    while read wheel; do
      ...
      curl -s -H "Authorization: bearer $token" \
        ...
        "https://s3.us.cloud-object-storage.appdomain.cloud/ose-power-artifacts-stag/$wheel"
